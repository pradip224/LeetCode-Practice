...
class Solution {
    public int compareVersion(String version1, String version2) {
        // Split the version numbers into individual revisions
        String[] revisions1 = version1.split("\\.");
        String[] revisions2 = version2.split("\\.");
        
        // Determine the maximum number of revisions
        int maxLength = Math.max(revisions1.length, revisions2.length);
        
        // Compare each corresponding revision of version1 and version2
        for (int i = 0; i < maxLength; i++) {
            // Get the integer value of the current revision (or treat it as 0 if not present)
